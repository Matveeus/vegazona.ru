		...
		<table class="tinvwl-table-manage-list">
			...
			<tr>
				...
				<?php if (isset($wishlist_table_row['colm_price']) && $wishlist_table_row['colm_price']) { ?>
					<th class="product-price table-cell-middle"><?php esc_html_e('Unit Price', 'ti-woocommerce-wishlist'); ?></th>
				<?php } ?>
				<?php if (isset($wishlist_table_row['colm_date']) && $wishlist_table_row['colm_date']) { ?>
					<th class="product-date table-cell-middle"><?php esc_html_e('Date Added', 'ti-woocommerce-wishlist'); ?></th>
				<?php } ?>
				<?php if (isset($wishlist_table_row['colm_stock']) && $wishlist_table_row['colm_stock']) { ?>
					<th class="product-stock table-cell-middle"><?php esc_html_e('Stock Status', 'ti-woocommerce-wishlist'); ?></th>
				<?php } ?>
				<?php if (isset($wishlist_table_row['add_to_cart']) && $wishlist_table_row['add_to_cart']) { ?>
					<th class="product-action table-cell-middle">&nbsp;</th>
				<?php } ?>
			</tr>
			</thead>
			<tbody>
			...
			foreach ($products as $wl_product) {
				...
				if ($wl_product['quantity'] > 0 && apply_filters('tinvwl_wishlist_item_visible', true, $wl_product, $product)) {
					...
					<tr class="<?php echo esc_attr(apply_filters('tinvwl_wishlist_item_class', 'wishlist_item', $wl_product, $product)); ?> ">
						...
						<td class="product-thumbnail product-thumbnail-block">
                            <div style="width: 120px; height: 90px; overflow: hidden;">
                                ...
                                if (!$product->is_visible()) {
                                    ...
                                    printf('<a href="#" class="yith-wcqv-button" data-product_id="%s">%s</a>', $product->get_id(), $thumbnail); // WPCS: xss ok.
                                }
                                ?>
                            </div>
						</td>
						<td class="product-name cart-tproduct-name">
							<?php
							if (!$product->is_visible()) {
								...
								echo apply_filters('tinvwl_wishlist_item_name', sprintf('<a href="#" class="yith-wcqv-button" data-product_id="%s">%s</a>', $product->get_id(), is_callable(array(
										$product,
										'get_name'
								)) ? $product->get_name() : $product->get_title()), $wl_product, $product); // WPCS: xss ok.
							}
						...
						<?php if (isset($wishlist_table_row['colm_price']) && $wishlist_table_row['colm_price']) { ?>
							<td class="product-price wishlist-tprice table-cell-middle">
								<?php
						...
						<?php } ?>
						<?php if (isset($wishlist_table_row['colm_stock']) && $wishlist_table_row['colm_stock']) { ?>
							<td class="product-stock table-cell-middle">
								<?php
						...
						<?php } ?>
						<?php if (isset($wishlist_table_row['add_to_cart']) && $wishlist_table_row['add_to_cart']) { ?>
							<td class="product-action table-cell-middle">
								<?php
								if (apply_filters('tinvwl_wishlist_item_action_default_loop_button', $wishlist_table_row['add_to_cart'], $wl_product, $product) && $product->is_in_stock()) {
